...
// Motor Controller Helper with Speed Matching
...
private:
  ...
  uint32_t lastUpdate;
  int16_t m2_pwm_adjustment;     // PWM adjustment for M2 to match M1
...
public:
  MotorController(RB_EncoderMotor* left, RB_EncoderMotor* right) 
    : masterMotor(left), slaveMotor(right), leftTargetSpeed(0), rightTargetSpeed(0), 
      lastUpdate(0), m2_pwm_adjustment(0), lastSpeedMatchUpdate(0) {}
  ...
  void setVelocity(float leftVel, float rightVel) {
    ...
    // Convert cm/s to PWM values
    int16_t leftPWM = (int16_t)constrain(leftVel * 2.0f, -99, 99);
    int16_t rightPWM = (int16_t)constrain(rightVel * 2.0f, -99, 99);
    
    // Apply adjustment to M2 to match M1 speed
    rightPWM += m2_pwm_adjustment;
    rightPWM = constrain(rightPWM, -99, 99);
    
    masterMotor->SetTarPWM(leftPWM);
    slaveMotor->SetTarPWM(rightPWM);
  }
  
  void updateSpeedMatching() {
    uint32_t now = micros();
    if ((now - lastSpeedMatchUpdate) < 100000) return; // Update every 100ms
    ...
    // Speed matching: adjust M2 PWM to match M1 speed
    if (abs(leftTargetSpeed) > 1.0f && abs(rightTargetSpeed) > 1.0f) {
      float speed_error = current_real_PPS_M1 - current_real_PPS_M2;
      
      // Simple proportional adjustment
      if (abs(speed_error) > 10.0f) {
        if (speed_error > 0) {
          m2_pwm_adjustment += 1; // M2 too slow, increase PWM
        } else {
          m2_pwm_adjustment -= 1; // M2 too fast, decrease PWM
        }
        m2_pwm_adjustment = constrain(m2_pwm_adjustment, -20, 20);
      }
    }
  ...
  // Get PWM adjustment
  int16_t getPWMAdjustment() { return m2_pwm_adjustment; }
...
void turnTo(float targetHeading) {
  ...
  while ((micros() - startTime) < timeout) {
    ...
    DEBUG_PRINTF("Turn: error=%.2f°, speed=%.1f, rate=%.3f°/ms, PPS:(%.1f,%.1f), PWM_adj:%d\n", 
                radToDeg(error), turnSpeed, current_turning_speed, 
                current_real_PPS_M1, current_real_PPS_M2, motorController.getPWMAdjustment());
...
void followPath(const Position* path, uint8_t pathSize, float lookahead, uint32_t endTime) {
  ...
  while (micros() < endTime) {
    ...
    DEBUG_PRINTF("Path: pos=(%.1f,%.1f), PPS=(%.1f,%.1f), steps=%ld, PWM_adj:%d\n",
                robotPose.x, robotPose.y, current_real_PPS_M1, current_real_PPS_M2, 
                current_m1_step_count, motorController.getPWMAdjustment());
...
void setup() {
  Serial.begin(115200);
  DEBUG_PRINTLN("Robot Tour V3 - Speed Matching & Enhanced Tracking");
  ...
  DEBUG_PRINTF("Final PWM Adjustment: %d\n", motorController.getPWMAdjustment());
  DEBUG_PRINTLN("===============================");
}
...
void loop() {
  ...
  if ((micros() - lastBlink) > MICROS_PER_SECOND) { // 1 second
    ...
    DEBUG_PRINTF("Pos:(%.1f,%.1f,%.1f°) M1_steps:%ld PPS:(%.1f,%.1f) Turn:%.3f°/ms PWM_adj:%d\n",
                robotPose.x, robotPose.y, radToDeg(robotPose.getHeading()),
                current_m1_step_count, current_real_PPS_M1, current_real_PPS_M2,
                current_turning_speed, motorController.getPWMAdjustment());
  }
